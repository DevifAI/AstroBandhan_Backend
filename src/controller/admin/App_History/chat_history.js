...
export const getChatRoom_History = asyncHandler(async (req, res) => {
    try {
        ...
        const chatHistory = await Promise.all(chatRooms.map(async (chatRoom) => {
            ...
            if (chat) {
                ...
                totalAmount = (durationInMinutes * chatRoom.astrologer.pricePerChatMinute) - (durationInMinutes * chatRoom.astrologer.chatCommission);
            }

            // Format start and end time to show only the time portion (HH:mm:ss)
            const startedTime = chatRoom.createdAt.toLocaleTimeString('en-US', { hour12: false });
            const endedTime = chatRoom.updatedAt.toLocaleTimeString('en-US', { hour12: false });
...
const {newFromDate, newToDate} = req.body
        ...
        if (!/^\d{2}-\d{2}-\d{4}$/.test(newFromDate) || !/^\d{2}-\d{2}-\d{4}$/.test(newToDate)) {
            return res.status(400).json({ message: "Invalid date format. Use dd-mm-yyyy for both 'from' and 'to' dates." });
        }
        ...
        const fromDate = new Date(`${fromYear}-${fromMonth}-${fromDay}T00:00:00.000Z`); // Start of the 'from' date
