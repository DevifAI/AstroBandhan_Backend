...
export const add_wallet_balance = asyncHandler(async (req, res) => {
  try {
    ...
    if (transaction_id_exist) {
      return res
        .status(201)
        .json(new ApiResponse(201, {}, `Invalid  Transaction Id !!`));
    }
    ...
    const add_wallet_doc = new Wallet({
      user_id: userId,
      amount,
      transaction_id,
    ...
    const add_admin_wallet_doc = new AdminWallet({
    ...
    if (userSocketId) {
      ...
      console.log(
        `Notification sent to user ${userId} with socket ID ${userSocketId}`
      );
    }
    ...
    return res
      ...
      .json(
        new ApiResponse(
          201,
          add_wallet_doc,
          `Money ${amount_type === "credit" ? "added" : "deduct"} successfully.`
        )
      );
  } catch (error) {
    return res
      ...
      .json(
        new ApiResponse(
          500,
          error,
          "An error occurred while working on the ballance."
        )
      );
  }
});
