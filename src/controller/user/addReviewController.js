    ...
    try {
        ...
        if (!astrologerId || !userId || !comment || rating == null) {
            return res.status(400).json(
                new ApiResponse(400, {}, "All fields (astrologerId, userId, comment, rating) are required.")
            );
        }
        ...
        const updatedAstrologer = await Astrologer.updateAverageRating(astrologerId, rating);

        if (!updatedAstrologer) {
            return res.status(500).json(
                new ApiResponse(500, {}, "Error updating astrologer rating.")
            );
        }
